...
class Dice(commands.Cog):
    ...
    def roll_dice(number, sides) -> List:
        if number == "":
            number = 1
        try:
            number = int(number)
    ...
    async def roll(self, ctx: commands.Context, *, args):
        ...
        try:
            ...
            await ctx.send(
                "There was an error with your formula. Please fix the number of dice and try again."
            )
            return
        except Dice.DiceFacesException:
            await ctx.send(
                "There was an error with your formula. Please fix the number/type of faces and try again."
            )
            return
        except:
